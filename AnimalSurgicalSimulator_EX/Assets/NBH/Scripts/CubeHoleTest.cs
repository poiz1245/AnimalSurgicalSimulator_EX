...
public class CubeHoleTest : MonoBehaviour
{
    ...
    Mesh GenerateMeshWithHoles()
    {
        ...
        for (int i = 0; i < origvertices.Length; ++i)
        {
            Vector3 v = new Vector3(origvertices[i].x * transform.localScale.x,
                                    origvertices[i].y * transform.localScale.y,
                                    origvertices[i].z * transform.localScale.z);
        ...
        triangles = RemoveAllSpecifiedIndicesFromArray(triangles, trianglesDisabled).ToArray();
    ...
    List<int> RemoveAllSpecifiedIndicesFromArray(int[] array, bool[] removeIndices)
    {
